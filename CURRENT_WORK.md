...
| 9.79 | Tool library | 🔵 S | **Green-lit.** Labs section; gate behind SiteSettings. Lending catalogue for volunteer/collective tools; availability status; contact-to-borrow MVP. See TASKS.md 9.79. |
...
| 9.88 | Shared shopping list (consumables) | 🟡 M | **Green-lit.** Labs section. Flag when items run out; volunteer pledges with ETA; supplier info and account-holder pointer. Bar stock excluded. See TASKS.md 9.88. |
| 9.89 | Weekly volunteer digest email | 🔵 S | **Green-lit.** Very useful for the effort. Opt-in per volunteer: upcoming shifts, new programme, starred events, shopping needs. Management command, not `MailoutJob`. `Volunteer.weekly_digest` + `last_digest_sent_at` fields. See TASKS.md 9.89. |
...
| ~~9.35.1~~ | ~~Toolkit homepage: dashboard section above link directory~~ | ✅ 2026-05-16 | Dashboard section above existing link directory. Three widgets: "Your upcoming shifts" (all volunteers — rota entries linked to current user; empty-state CTA to browse rota), "Your starred events" (future showings only; hidden when empty), "New since your last login" (Programmer+ only; showings added since last login, capped at 30-day lookback; hidden when empty). All computed in `ToolkitIndexView.get_context_data()`; no new views. Shopping list widget (widget 4) deferred pending 9.88. 10 new tests. |
...
| 9.58 | Programme: one card per showing | 🟡 M | **Decided:** one card per showing (not stacked). Bold date = this showing's next date. Need to integrate with the "next showing" bolding logic carefully. See TASKS.md 9.58. |
