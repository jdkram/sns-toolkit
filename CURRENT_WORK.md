...
| 9.49 | Programmer access to templates + Panopticon-only roles | ✅ 2026-03-02 | `edit_roles` gated on `is_superuser`; Programmers group + demo user in `configure_toolkit_users`; index shows diary/meta-programming to `perms.toolkit.write`, roles/volunteers/members to superusers; access-tier label at bottom of index; cascade-deletion + rename warnings on roles page; shared-state warnings on template pages; SPEC.md section 2 updated |
