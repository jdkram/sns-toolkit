...
| 9.87 | Collectives → simplelists email list sync | 🟡 M (spec) | Joining a collective subscribes to the list; leaving unsubscribes. Uses simplelists subscribe/unsubscribe form POST (no API key available). Existing manual subscriptions left alone. See TASKS.md 9.87 for design notes. |
| 9.88 | Shared shopping list (consumables) | 🟡 M (spec) | Flag when items run out; volunteer pledges with ETA; supplier info and account-holder pointer; Labs section. Bar stock excluded for now. See TASKS.md 9.88 for full design. |
| 9.89 | Weekly volunteer digest email | 🔵 S (spec) | Opt-in weekly email per volunteer: upcoming shifts, new programme, starred events, shopping needs. Personalised management command, not `MailoutJob`. `Volunteer.weekly_digest` + `last_digest_sent_at` fields. See TASKS.md 9.89. |
| 9.90 | Access transparency: privileged user list + rights explanation | 🔵 S (spec) | Visible list of all Panopticon and Programmer users; plain-language rights table per tier; `PanopticonGrant` model captures reason + date + annual review flag. Governance note: share access level descriptions with collective before deploying. See TASKS.md 9.90. |
...
| 9.33 (view) | Diary column-per-room view | 🟡 M | Calendar "week (rooms)" view exists in FC6; the diary list view doesn't have a column-per-room layout yet. |
...
| 9.63 | Interactive SVG venue map for room bookings | 🟡 M | SVG map integration in showing edit advanced panel is done (map loads via fetch, JS toggles room selection). Room availability overlay (showing existing bookings on the map) is out of scope for this ticket. |
