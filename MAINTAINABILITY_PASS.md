...
### Chunk 10: Squash migrations — DONE 2026-06-28
...
Order:
- [x] Squash diary 0009-0035 → diary 0009_v2 — DONE 2026-06-28. Foundation layer (rota/role/template/showing/room fields + the SiteConfiguration model creation in 0019). Folds 5 data migrations locally: `copy_roles_to_through_model` (0014, copies the old auto M2M into the new EventTemplateRole through model — noop on fresh install), `seed_singleton` (0019, creates the SiteConfiguration pk=1 row seeded from settings — NOT a noop, essential), `_add_column`/`_drop_column` (0029, MySQL IF-NOT-EXISTS guard for `calendar_slot_min_hour`), `backfill_room_bookings` (0032, noop on fresh install), `set_map_slugs` (0034, noop without seeded rooms). Deps: diary.0008 (pre-baseline) + members.0009_v2 (repointed from members.0010/0013). 27 → 1 file.
- [x] Squash diary TMDB cycle 0075-0083 → folded into the 0036-0090 squash below. The add-then-remove TMDB fields did NOT collapse cleanly (143 → 135 ops only) because `update_guidance` (0081) sits inside the cycle and acts as an optimizer barrier. The final Film model carries OMDb fields (not TMDB); the intermediate TMDB add/remove operations remain in the squash but are schema-only and net out correctly. Verified by the suite.
- [x] Squash diary SiteConfiguration field-add migrations 0036-0090 → diary 0036_v2 — DONE 2026-06-28. Folds 10 data migrations locally (0047 seed_new_fields, 0049 set_map_slugs, 0050 carry_over_dormancy_value, 0060/0061/0062 MySQL IF-NOT-EXISTS column guards, 0067 copy_notes_to_programming_notes, 0070 seed_bbfc_defaults, 0071 seed_ticketsource_guidance, 0081 update_guidance). Deps: diary.0009_v2 + members.0009_v2 (repointed from diary.0035/members.0022). **One optimizer pitfall caught and fixed:** 0086 added `perm_diary_edit`/`perm_programming_queue` with `default='programmer'` (so the seeded pk=1 row got 'programmer'), then 0087's `AlterField` changed the column default to 'volunteer' without touching existing rows. The squash optimizer merged each into a single `AddField(default='volunteer')`, which would have set pk=1 to 'volunteer' and loosened diary-edit-list access to all volunteers (caught by `ViewSecurity.test_rota_editor_no_access`). Fix: split each back into `AddField(default='programmer')` + `AlterField(default='volunteer')` to reproduce the original semantics (existing row keeps 'programmer'; column default becomes 'volunteer' for future inserts). 55 → 1 file.
- [x] Squash members 0009-0028 → members 0009_v2 — DONE 2026-06-28. (Plan said 0009-0029; the last members migration is 0028, not 0029 — the range is 20 files either way.) Folds 2 data migrations locally: `populate_status_from_active` (0012, carries `active=False` → `status='retired'`; the `active` field is removed later in 0018, and the RunPython runs before that RemoveField so the column still exists — noop on fresh install), `split_dir_share_name`/`recombine_dir_share_name` (0015, splits the `dir_share_name` enum into `dir_share_listed`+`dir_share_name_style` then drops the old field; RunPython runs before the RemoveField). Sets `atomic = False` (inherited from 0015 — MariaDB can't do DDL inside a transaction with data migrations). Deps: labs.0001_initial_v2 (repointed from labs.0009 via the members.0014 → labs.0009 cross-app dep) + members.0008 (pre-baseline). 20 → 1 file.
...
- [x] mailer + index already single-file additions; left alone. mailer has 0002, index has 0003 — both single follow-on migrations, not worth squashing.

**Chunk 10 DONE 2026-06-28.** Net: 178 → 23 migration files (139 deleted, 6 squashes added across 5 apps). Verified: 959 tests pass; black --line-length 79 clean on all new files.
